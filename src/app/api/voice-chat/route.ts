...
// Hospital-specific system prompt
const SYSTEM_PROMPT = `You are AURA, the friendly AI assistant at Mutare Provincial Hospital in Zimbabwe.
You help patients navigate the hospital and answer health-related questions.
Keep responses SHORT (2-3 sentences max) — the response will be spoken aloud to a patient at a kiosk.
If the patient's request clearly maps to a kiosk section, include a JSON navigation hint at the end like:
  [NAVIGATE:/kiosk/doctors]
...
Respond in the same language the patient uses if possible. Be warm, calm, and professional.
Do NOT include the [NAVIGATE:...] tag if navigation is not clearly needed.`
...
export async function POST(request: Request) {
  try {
    ...
    // Sanitize: truncate input to prevent abuse
    const safeMessage = message.slice(0, 500).trim()

    if (!GEMINI_API_KEY) {
      // Graceful fallback when no key configured: simple keyword routing
    ...
    // Extract optional navigation hint
    ...
    return NextResponse.json({ reply: 'Something went wrong. Please try again or select an option on screen.' })
  }
}
...
function fallbackResponse(message: string): { reply: string; navigate?: string } {
  ...
  if (m.includes('toilet') || m.includes('bathroom') || m.includes('map') || m.includes('facilit') || m.includes('where'))
  ...
  if (m.includes('fee') || m.includes('cost') || m.includes('price') || m.includes('pay'))
    return { reply: "Here's the hospital fee and cost information.", navigate: '/kiosk/information?tab=fees' }
  if (m.includes('queue') || m.includes('wait') || m.includes('ticket') || m.includes('number'))
